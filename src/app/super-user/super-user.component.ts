...
export class SuperUserComponent implements OnInit {
  ...
  getDashboardData() {
    this.service.showNumberOfUsers().subscribe(
      (data: any) => {
        console.log('Get number of users', data);
        this.totalOperator = data.operators;
        this.totalAdmin = data.admins;
      },
      (error: any) => {
  ...
  createChart() {
    ...
    this.myChart2 = echarts.init(document.getElementById('graph_area2') as any);
  ...
  editAdmin(userType: any) {
    ...
    if (employee_id == '' || new_password == '' || cnf_password == '') {
      ...
      if (new_password === cnf_password) {
        ...
        this.service.modifyAdminOperatorPassword(newData).subscribe(
          (data: any) => {
